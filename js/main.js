(function (win, doc, undefined) {
    ...
    function closest(ele, tar) {
        ...
        var elArr = (function () {
            if (tar instanceof HTMLElement) return [tar]; try { tar = doc.querySelectorAll(tar); } catch (e) { console.log(e); }
            if (['NodeList', 'HTMLCollection', 'Array'].indexOf(whichType(tar)) > -1) return toArray(tar);
        })(); do { if (elArr.indexOf(ele) > -1) return ele; ele = ele.parentElement; } while (ele !== null); return null;
    }
    ...
    function getTransitionEventName() {
        var el = document.createElement('div'); var transitions = { 'transition': 'transitionend', 'WebkitTransition': 'webkitTransitionEnd', 'OTransition': 'oTransitionEnd', 'MozTransition': 'transitionend' }
        for (var attr in transitions) { if (el.style[attr] !== undefined) return transitions[attr]; }
    }
    function insertCssRules(rules) {
        var sheets = toArray(doc.styleSheets), lastSheet = null; sheets = sheets.filter(function (e) { return !isTrue(e.disabled) && e.ownerNode.tagName.toLowerCase() === 'style'; })
    ...
    function setParams(params) {
        extend(params, defaults); var autoplay = params.autoplay; if (isTrue(autoplay)) { params.autoplay = { enable: true, delay: 7000 } }
    ...
    function appendStyleRules() {
        var backdrop = this, wrapper = backdrop.wrapper, params = backdrop.params, slides = backdrop.slides, length = slides.length, mediant = slides.mediant, dataAttrArr = slides.dataAttrArr; var cssRuleArr = []; function calcSlideCssRule(index) {
            var relativeI = index - mediant, absN = Math.abs(relativeI), zIndex = mediant - absN; var cssObj = {}, wrapperSizeObj = wrapper.getBoundingClientRect(); cssObj['z-index'] = params.baseZIndex + zIndex; if (params.opacity) { cssObj['opacity'] = Math.pow(params.opacity, absN); }
    ...
    var Autoplay = {
        init: function () { var backdrop = this, enable = backdrop.params.autoplay.enable; enable && backdrop.autoplay.run(); }, run: function () { var backdrop = this, delay = backdrop.params.autoplay.delay; backdrop.autoplay.stop(); backdrop.autoplay.timer = setInterval(function () { backdrop.slideToNext(); }, delay); }, stop: function () {
            var backdrop = this; if (backdrop.autoplay.timer) { clearInterval(backdrop.autoplay.timer); backdrop.autoplay.timer = undefined; }
            return true;
        }
    }
    ...
    var Click = {
        canClickEles: [], setCanClickEles: function () { var backdrop = this, navigation = backdrop.params.navigation, slides = backdrop.slides, canClickEles = backdrop.click.canClickEles; Object.keys(navigation).forEach(function (e) { !isNull(navigation[e]) && canClickEles.push(navigation[e]); }); isTrue(slides.clickable) && canClickEles.push(slides.list); backdrop.click.canClickEles = flatArray(canClickEles, Infinity); }, init: function () { var backdrop = this; backdrop.click.setCanClickEles(); if (isEmptyArray(backdrop.click.canClickEles)) return; backdrop.click.run(); }, run: function () {
            var backdrop = this, wrapper = backdrop.wrapper, canClickEles = backdrop.click.canClickEles, navigation = backdrop.params.navigation, waitForTransition = backdrop.params.transition.waitForTransition; wrapper.parentElement.addEventListener('click', function (ev) {
                var target = ev.target, willTar = null, ele = null; for (var i = 0, len = canClickEles.length; i < len; i++) { ele = canClickEles[i]; if (ele === closest(target, ele)) { willTar = ele; break; } }
                if (isNull(willTar)) return; if (waitForTransition) {
